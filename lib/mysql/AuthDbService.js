...
function saveUserInfo(userInfo, skey, session_key) {
    ...
    return mysql('cSessionInfo').count('open_id as hasUser').where({
        ...
        // 这里是构建了字段名与变量名一致，来减少键值对书写ES6简写语法
        // knex的 Query Builder模式
        .then(res => {
            // 如果存在用户则更新
            if (res[0].hasUser) {
        ...
        // 所以外层$wxInfo==result==loginState,userInfo==>{ userinfo: userInfo,skey: skey}
        // 由于大括号被解释为代码块，如果箭头函数直接返回一个对象，必须在对象外面加括号
        .then(() => ({
            userinfo: userInfo,
            skey: skey
        }))
